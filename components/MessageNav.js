...
const MessageNav = () => {
  ...
  return (
    <>
      ...
      <nav
        className={`bg-white shadow-lg flex flex-col w-1/4 items-start px-6 py-4 fixed top-0 left-0 right-0 z-50 sm:hidden ${
          mobileNavOpen ? 'visible' : 'invisible'
        }`}
      >
        <p className="text-[19px] tracking-widest font-bold mt-10 mb-6">Conversation</p>
        <Link href="/"><p className="text-[12px] w-full text-center bg-blue-100 px-9 py-2 rounded-md cursor-pointer mb-2">Dashboard</p></Link>
      ...
      <nav className="app bg-white shadow-lg flex items-center justify-between px-6 h-25 fixed top-0 sm:left-24 left-0 right-0 z-50">
